...
function HomeIcon({ iconKey, gradient, label, onClick, index = 0 }) {
  return (
    <button onClick={onClick} aria-label={`Open ${label}`} className="home-icon"
    ...
    return (
      <div className="mobile-app" style={{ position: 'fixed', inset: 0, zIndex: 99999, display: 'flex', flexDirection: 'column', background: '#15151c' }}>
        <div style={{
          ...
          <button onClick={() => setApp(null)} style={{ background: 'none', border: 'none', color: '#40c4ff', fontSize: 15, cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 4 }}>
            ‹ Home
          </button>
  ...
  return (
    <div className="mobile-home" style={{ position: 'fixed', inset: 0, zIndex: 99999, background: wallpaper, display: 'flex', flexDirection: 'column', overflowY: 'auto' }}>
      <div style={{ padding: '46px 24px 10px', textAlign: 'center' }}>
        ...
        <p style={{ color: 'rgba(255,255,255,0.8)', fontSize: 13, marginTop: 4, fontFamily: "'Inter', sans-serif" }}>{IDENTITY.role}</p>
        <p style={{ color: 'rgba(255,255,255,0.55)', fontSize: 11.5, marginTop: 3, fontFamily: "'JetBrains Mono', monospace" }}>{IDENTITY.location}</p>
      </div>
      ...
      <div style={{ flex: 1, display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '24px 8px', padding: '24px 20px', alignContent: 'start' }}>
        ...
        {DOCK_EXTERNALS.map((ext, i) => (
          <HomeIcon key={ext.label} index={DOCK_ITEMS.length + i} iconKey={ext.label} gradient={ext.gradient} label={ext.label} onClick={() => window.open(ext.url, '_blank')} />
        ))}
      </div>

      <div style={{ textAlign: 'center', padding: '8px 0 18px', color: 'rgba(255,255,255,0.45)', fontSize: 11, fontFamily: "'JetBrains Mono', monospace" }}>
        tap an app to open
      </div>
    </div>
  );
