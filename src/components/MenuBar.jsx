...
const BatteryIcon = () => (
  <svg width="20" height="11" viewBox="0 0 28 13" fill="currentColor">
...
// Build the menu → item map from the actions handed down by App.
function buildMenus({ activeId, wins, wallpaper, actions }) {
  ...
  return {
    ...
    Help: [
      { label: 'Portfolio on GitHub', onClick: () => window.open(IDENTITY.github, '_blank') },
      { label: 'Email Joshua', onClick: () => window.open(`mailto:${IDENTITY.email}`) },
    ],
  };
}
...
function Dropdown({ items, onPick }) {
  return (
    <div className="menu-pop" style={{
      ...
      background: 'rgba(40,36,60,0.78)', backdropFilter: 'blur(20px) saturate(180%)',
      ...
      {items.map((it, i) =>
        it.divider ? (
          <div key={i} style={{ height: 1, background: 'rgba(255,255,255,0.12)', margin: '5px 8px' }} />
        ) : (
          <div
            key={i}
            ...
            style={{
              ...
              color: it.disabled ? 'rgba(255,255,255,0.35)' : '#f0f0f5',
  ...
  return (
    <>
      ...
      <div style={{
        ...
        background: 'rgba(20,16,34,0.55)', backdropFilter: 'blur(24px) saturate(180%)',
        ...
        {labels.map((label, i) => {
          ...
          return (
            <div key={label} style={{ position: 'relative' }}>
              ...
              {open && <Dropdown items={menus[label]} onPick={() => setOpenMenu(null)} />}
            </div>
          );
        })}
